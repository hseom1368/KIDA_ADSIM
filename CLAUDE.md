...
## 다음 작업: v0.5
1. **COP 품질 차별화** — Linear: 위협 항적만 공유 / Kill Web: 위협 + 아군 자산 상태 + 대응 계획 공유 → 센서 융합 √N 오차 감소, 사수 재할당 최적화
2. **적응형 교전 정책** — 잔여 탄약 임계값 이하 시 다중→단일 교전 자동 전환
3. **통신 네트워크 동적 열화** — 재밍 환경 링크별 차등 열화 (SNR 기반)
4. **2D 전술 시각화 모듈** — matplotlib.animation 기반 전투공간 리플레이 (에이전트 배치, 이동 궤적, 교전 이벤트, 센서/사수 커버리지)
5. **시각화 노트북** — notebook5 신규: 시나리오별 전술 애니메이션 + 비교 스냅샷
...
- `plan.md` — 현재 진행 중인 개선 작업의 상세 기술 계획 (현재: v0.5)
