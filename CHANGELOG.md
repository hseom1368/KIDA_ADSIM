...
#### 전술 시각화 GIF 생성
- 전 시나리오(7개) × 2 아키텍처 = **14개 GIF 애니메이션** 생성
- 시나리오별 초기/중기/후기 = **21개 비교 스냅샷 PNG** 생성
- `output/gif/` 디렉터리에 저장 (GitHub에서 직접 다운로드 가능)

#### 테스트 확장
- 116개 → **136개** (신규 20개, 15개 파일)
...
- `output/` 디렉터리가 `.gitignore`에 포함 — GIF 파일은 `git add -f`로 강제 추가

### 개선 계획
- **v0.7**: 구조적 개선 — 10개 항목 (아래 상세)
  1. 표적 분류 + KN-25 오분류 모델
...
- **v0.8+**: 확장 체계 (AEW&C, Aegis, SBIRS, L-SAM, 비용 모델, 지형 차폐)
...
## [v0.2-dev] — 2026-03-09 (분석 완료, v0.2에 통합)

### 변경사항
- (코드 변경 없음 — 분석 및 계획 수립, v0.2에서 구현)
